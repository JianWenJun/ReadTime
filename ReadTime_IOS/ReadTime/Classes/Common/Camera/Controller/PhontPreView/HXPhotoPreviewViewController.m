...
- (void)viewDidLoad {
    ...
    self.title = @"预览";
    [self setRithtItemWithTitle:@"删除" selector:@selector(delBtnAction:)];
...
- (void)initUI {
    //photoScrollview
    _photoScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, CONTENT_HEIGHT)];
    _photoScrollView.pagingEnabled = YES;
    _photoScrollView.delegate = self;
    _photoScrollView.showsHorizontalScrollIndicator = NO;
    _photoScrollView.showsVerticalScrollIndicator = NO;
    _photoScrollView.backgroundColor = UIColor.clearColor;
    _photoScrollView.contentSize = CGSizeMake(SCREEN_WIDTH * _photos.count, 0);
    [self.view addSubview:_photoScrollView];
}

- (UILabel *)countLabel {
    if (!_countLabel) {
        _countLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, CONTENT_HEIGHT - 18 - 39, SCREEN_WIDTH, 18)];
