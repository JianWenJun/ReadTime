...
- (void)addTakePictureButton {
    ...
    _takePictureButton.frameCenterX = SCREEN_WIDTH/2;
...
- (void)addCutTakeTypeButton {
    ...
    _cutTakeTypeButton.frameOriginX = SCREEN_WIDTH - _cutTakeTypeButton.frameSizeWidth - 30;
...
- (void)sellTitleViewForTitle:(NSString *)title {
    ...
    if (StatusBarStyle == UIStatusBarStyleLightContent) {
        icon = [icon imageToColor:UIToneTextColor];
    }
...
- (UIView *)ittemRedViewWithRedDotValue:(NSString *)redDotValue {
    ...
    if (UIToneTextColor == [UIColor whiteColor]) {
        label.backgroundColor = [UIColor redColor];
        label.textColor = [UIColor whiteColor];
    } else {
        label.backgroundColor = UIToneTextColor;
        label.textColor =UIToneBackgroundColor;
    }
...
- (void)showAllFooterView {
    self.footerView.frame = CGRectMake(0, CONTENT_HEIGHT - (CONTENT_HEIGHT - CardSwitchViewHeight), SCREEN_WIDTH, CONTENT_HEIGHT - CardSwitchViewHeight);
    self.collectionView.frame = CGRectMake(0, 10, SCREEN_WIDTH, self.footerView.frame.size.height - 10);
}
...
- (void)gotoCutPhotoViewController {
    ...
    cropSize = CGSizeMake(SCREEN_WIDTH, SCREEN_WIDTH);
...
- (UITableView *)dropDownTableView {
    if (!_dropDownTableView) {
        _dropDownTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, self.view.frame.size.height) style:UITableViewStylePlain];
...
- (HXCardSwitchView *)cardSwitchView {
    if (!_cardSwitchView) {
        ...
        _cardSwitchView.layout.itemSize = CGSizeMake(SCREEN_WIDTH - 40, 243);
...
- (UIView *)footerView {
    if (!_footerView) {
        float height = (SCREEN_WIDTH - 5*6) / 5 + 30;
        
        _footerView = [[UIView alloc] initWithFrame:CGRectMake(0, CONTENT_HEIGHT - height, SCREEN_WIDTH,height)];
