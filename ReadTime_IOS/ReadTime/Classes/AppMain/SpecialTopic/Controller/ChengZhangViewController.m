...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self.tableview registerNib:[UINib nibWithNibName:@"ChengZhangCell" bundle:nil] forCellReuseIdentifier:@"ChengZhangCell"];
...
#pragma mark - 数据加载
- (void)loadListData{
    [SpecialArticleDataEngine control:self skip:1 typeID:7 complete:^(id data, NSError *error) {
        if (error) {
...
- (void)currentViewDothing{
    ...
    NSLog(@"ChengZhangViewController--doing");
}
- (void)disCurrentViewDothing{
    NSLog(@"ChengZhangViewController--undoing");
