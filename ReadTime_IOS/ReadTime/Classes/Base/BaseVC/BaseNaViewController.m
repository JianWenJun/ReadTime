...
@interface BaseNaViewController ()<UIGestureRecognizerDelegate>
...
- (void)loadView{
    [super loadView];
    [self.navigationBar setBackgroundImage:[UIImage imageWithColor:UIToneBackgroundColor size:CGSizeMake(self.navigationBar.frame.size.width, self.navigationBar.frame.size.height+20)]
                            forBarPosition:UIBarPositionAny
                                barMetrics:UIBarMetricsDefault];
    [self.navigationBar setShadowImage:[UIImage new]];
    
    //    [[UIApplication sharedApplication] setStatusBarStyle:StatusBarStyle];
    self.navigationBar.titleTextAttributes=@{NSForegroundColorAttributeName:UIToneTextColor,
                                             NSFontAttributeName:[UIFont systemFontOfSize:18]};
    if ([UIDevice currentDevice].systemVersion.floatValue>7.0) {
        self.navigationBar.barTintColor=UIToneBackgroundColor;
    }
    //系统返回按钮图片设置
    ...
    [[UINavigationBar appearance] setTintColor:UIToneTextColor];
...
- (NSMutableArray *)rootVcAry {
    if (!_rootVCArry) {
        _rootVCArry = [NSMutableArray new];
    }
    return _rootVCArry;
}
