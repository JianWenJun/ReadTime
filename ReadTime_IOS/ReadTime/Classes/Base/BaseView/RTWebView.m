...
@interface RTWebView ()<UIWebViewDelegate,WKNavigationDelegate,WKUIDelegate,
IMY_NJKWebViewProgressDelegate>
...
- (instancetype)init{
    return [self initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT-64)];
}
...
- (void)_initMyself{
    ...
    if (wkWebView && self.usingUIWebView == NO) {
        [self initWKWebView];
        _usingUIWebView = NO;//初始化WKWebView
    }else {
        [self initUIWebView];
        _usingUIWebView = YES;//初始化UIWebView
    }
...
///--  还没用到
#pragma mark - CALLBACK RTWebView Delegate
